...
async def is_admin(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    try: return any(a.user.id == u.effective_user.id for a in await c.bot.get_chat_administrators(u.effective_chat.id))
    except: return False
...
async def start_deal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    s_fmt, b_fmt = await res_uid(f["seller"] or "N/A", rep, c, u.effective_chat.id), await res_uid(f["buyer"] or "N/A", rep, c, u.effective_chat.id)
...
async def close_deal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if PROOF_CHANNEL:
        try: await c.bot.send_message(chat_id=PROOF_CHANNEL, text=sm.text, parse_mode="HTML")
        except: pass
...
async def status_deal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if did in DEALS_DB:
        ...
        return
    if raw_rep:
        f, st = extract_f(raw_rep), ("COMPLETED" if "completed" in raw_rep.lower() else ("ON HOLD" if "hold" in raw_rep.lower() else ("CANCELLED" if "cancelled" in raw_rep.lower() else "ACTIVE")))
...
async def handle_edited_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if any(a.user.id == em.from_user.id for a in await c.bot.get_chat_administrators(em.chat_id)): return
    except: pass
    try:
        await em.delete()
...
async def handle_txt(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if fm and parse_amt(fm.group(1)) > 0: await send_fee_result(u, parse_amt(fm.group(1)))
