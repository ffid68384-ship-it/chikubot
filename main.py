...
async def cmd_deal(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if LAST_PIN:
        try:
            await c.bot.unpin_chat_message(chat_id=u.effective_chat.id, message_id=LAST_PIN)
        except:
            pass
...
async def cmd_received(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not did:
        ...
        if row:
            did, amt, seller, buyer = row[0], (amt or row[1]), (seller or row[2]), (buyer or row[3])
    amt_lbl = f"₹{amt:,.0f}" if amt > 0 else "Deal Amount"
...
async def cmd_close(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not did:
        ...
        if row:
            did, amt, seller, buyer, r_mid = row[0], (amt or row[1]), (seller or row[2]), (buyer or row[3]), row[4]
    if c.args:
        n = re.sub(r'[^\d\.]', '', c.args[0])
        if n and float(n) > 0:
            amt = float(n)
    ...
    if did:
        db_run('UPDATE deals SET status = "COMPLETED" WHERE did = ?', (did,))
...
async def cmd_cancel(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if LAST_PIN:
        try:
            ...
            pass
    sm = await c.bot.send_message(chat_id=u.effective_chat.id, text=f"❌ <b>DEAL CANCELLED</b>\n🪪 <b>ID:</b> {did}\n👤 <b>By:</b> {u.effective_user.mention_html() if u.effective_user else 'Admin'}", parse_mode="HTML")
    try:
        await c.bot.pin_chat_message(chat_id=u.effective_chat.id, message_id=sm.message_id, disable_notification=True)
        LAST_PIN = sm.message_id
    except:
...
async def cmd_refund(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not did:
        ...
        if row:
            did, amt, buyer = row[0], (amt or row[1]), (buyer or row[2])
    if c.args:
        n = re.sub(r'[^\d\.]', '', c.args[0])
        if n and float(n) > 0:
            amt = float(n)
